...
import { ShopKeeperRepository, ShopkeeperService } from './shopkeeper.service';
...
describe('ShopkeeperService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ShopkeeperService,
        {provide: ShopKeeperRepository, useFactory: () => ShopKeeperRepositoryStub}
      ],
    }).compile();
  ...
  describe('getShopKeeper', () => {
    ...
    it('Should throw if repository throws', async () => {
      (repository.getShopKeeper as jest.Mock).mockRejectedValueOnce(new InternalServerErrorException());
      await expect(service.getShopKeeper(4)).rejects.toThrow(new InternalServerErrorException());
    })
  ...
  describe('createShopKeeper', () => {
    ...
    it('should throw if repository throws', async () => {
      (repository.createShopKeeper as jest.Mock).mockRejectedValueOnce(new InternalServerErrorException());
